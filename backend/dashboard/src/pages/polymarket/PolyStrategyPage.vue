...
async function refreshCustom() {
  try {
    const resp = await api.getCustomStrategies('POLYMARKET')
...
function computeBreakdown(trades) {
  ...
  for (const t of trades) {
    const key = t.market || t.symbol || 'Unknown'
    if (!breakdown[key]) {
...
<template>
  ...
  <div class="tp-page strat-page">
    ...
    <div class="page-header">
      <div>
        <h1>Polymarket Strategies</h1>
        <p>Manage and backtest your Polymarket prediction strategies.</p>
      </div>
    ...
    <!-- Strategy Builder -->
    <StrategyBuilder v-if="showBuilder" domain="POLYMARKET" @saved="onBuilderSaved" @cancel="showBuilder = false" />
      ...
      <div class="tp-card strategy-card">
        ...
        <div class="card-top">
          <div class="card-title-row">
            <div class="strat-icon" style="background:rgba(6,182,212,0.15);color:#06b6d4;">
              <span class="material-symbols-outlined" style="font-size:24px">casino</span>
            </div>
            <div>
              <h3 class="strat-name">EV Prediction Strategy</h3>
              <p class="strat-desc">Expected Value-based Polymarket predictions</p>
            </div>
          </div>
        ...
        <div class="card-stats">
          <div class="stat-item">
            <span class="stat-micro-label">EV Threshold</span>
            <span class="stat-micro-value">{{ config.ev_threshold }}</span>
          </div>
          <div class="stat-item">
            <span class="stat-micro-label">Kelly Fraction</span>
            <span class="stat-micro-value">{{ config.kelly_fraction }}</span>
          </div>
          <div class="stat-item">
            <span class="stat-micro-label">Max Positions</span>
            <span class="stat-micro-value">{{ config.max_positions }}</span>
          </div>
        </div>
        ...
        <div class="card-expandable">
            ...
            <div class="expand-content">
              <table class="detail-table">
                <tbody>
                  ...
                  <tr><td>Run Time</td><td>{{ fmtDate(latestBacktest.created_at ?? latestBacktest.run_time) }}</td></tr>
                </tbody>
              </table>
            </div>
          </details>
          ...
          <details v-if="latestBacktest && latestBacktest.trades && latestBacktest.trades.length">
            ...
            <div class="expand-content" style="overflow-x: auto;">
              <table class="detail-table">
                <thead>
                  <tr><th>#</th><th>Market</th><th>Side</th><th>Entry</th><th>Exit</th><th>Shares</th><th>PnL</th></tr>
                </thead>
                <tbody>
                  <tr v-for="(t, i) in latestBacktest.trades" :key="i">
                    ...
                    <td>{{ t.market ?? '-' }}</td>
                    <td>
                      <span class="tp-badge" :class="t.side === 'YES' ? 'tp-badge-success' : 'tp-badge-danger'" style="font-size:0.65rem;">
                        {{ t.side ?? '-' }}
                      </span>
                    </td>
          ...
          <!-- Charts (Equity Curve + Symbol Breakdown — NO TradeMarkersChart) -->
          <details v-if="latestBacktest">
            <summary class="expand-summary">
            ...
            <div class="expand-content">
              <div class="chart-tabs">
                ...
                <button class="tp-btn tp-btn-outline" :class="{ 'tp-btn-primary': activeChartTab === 'breakdown' }" @click="activeChartTab = 'breakdown'">Market Breakdown</button>
              </div>
              <EquityCurveChart v-if="activeChartTab === 'equity'" :equity-curve="latestBacktest.equity_curve || []" />
              <SymbolBreakdownTable v-if="activeChartTab === 'breakdown'" :breakdown="computeBreakdown(latestBacktest.trades)" />
            </div>
          </details>
      ...
      <div v-else class="strategy-grid">
        <div v-for="(cs, idx) in customStrategies" :key="'c'+cs.id" class="tp-card strategy-card">
          ...
          <div class="card-expandable">
            ...
            <!-- Charts (Equity Curve + Breakdown — no TradeMarkersChart for Polymarket) -->
            <template v-if="cs.latest_backtest">
              <details>
                <summary class="expand-summary">
                  <span class="material-symbols-outlined" style="font-size:16px">ssid_chart</span>
                ...
                <div class="expand-content">
                  <div class="chart-tabs">
                    ...
                    <button class="tp-btn tp-btn-outline" :class="{ 'tp-btn-primary': activeTab[cs.id] === 'breakdown' }" @click="setTab(cs.id, 'breakdown')">Market Breakdown</button>
                  </div>
                  <EquityCurveChart v-if="!activeTab[cs.id] || activeTab[cs.id] === 'equity'" :equity-curve="cs.latest_backtest.equity_curve || []" />
                  <SymbolBreakdownTable v-if="activeTab[cs.id] === 'breakdown'" :breakdown="computeBreakdown(cs.latest_backtest.trades)" />
                </div>
              </details>
            </template>
          </div>
        ...
        <div class="tp-add-card" @click="showBuilder = true">
          ...
          <p style="font-size:0.85rem;text-align:center;max-width:220px;">Use the visual builder to create and backtest Polymarket strategies.</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="pageTab === 'library'" style="margin-top: 1rem;">
      <StrategyLibrary domain="POLYMARKET" />
    </div>
  </div>
</template>
