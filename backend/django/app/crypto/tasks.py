...
def run_lighter_adx_entry():
    if is_crypto_bot_paused():
        return
    try:
        from app.quant.algorithms.lighter.adx_entry import adx_entry_algorithm
        adx_entry_algorithm()
    except SoftTimeLimitExceeded:
        logger.error("run_lighter_adx_entry timed out.")
    except Exception as e:
        logger.error("run_lighter_adx_entry error: %s", e)
